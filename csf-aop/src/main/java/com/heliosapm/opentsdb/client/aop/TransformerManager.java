...
public class TransformerManager {
	/** Static class logger */
	private static final Logger LOG = LogManager.getLogger(TransformerManager.class);
	/** The known providers */
	private static final List<InstrumentationProvider> PROVIDERS = Collections.unmodifiableList(new ArrayList<InstrumentationProvider>(Arrays.asList(
			DefaultInstrumentationProvider.INSTANCE, JBossAOPInstrumentationProvider.INSTANCE, 
			StaticFieldInstrumentationProvider.INSTANCE, JMXAttributeInstrumentationProvider.INSTANCE
	)));
	 ...
	 * @param instrumentation The optional instrumentation instance to use. If null, will attempt to locate the instance
	 * using {@link #getInstrumentation()}.
	 */
	...
	/**
	 * Attempts to execute each known {@link InstrumentationProvider} until all fail or an {@link Instrumentation} instance is found.
	 * @return an {@link Instrumentation} instance or null if one could not be found
	 */
	public static Instrumentation getInstrumentation() {
		for(InstrumentationProvider ip: PROVIDERS) {
			try {
				return ip.getInstrumentation();
			} catch (Exception ex) {
				/* No Op */
			}
		}
